...
# this method runs a prediction run for a trained neural network directly predicting the 2m-temperature
def runModel(config, data_dictionary, data_statistics, train_test_folds):
    ...
    for run in range(config['runs']):
        ...
        dataset = DataLoaders.CosmoDataGridData(
            config=config,
        ...
        run_error_statistics = np.empty((len(init_index_dict), len(station_index_dict), 4))
        ...
        for i, data in enumerate(dataloader, 0):
            try:
                ...
                DATA = data
                # DATA has only length 2 if we do not use the station time invariant features
                if len(DATA) == 4:
            ...
            for item in range(Blabel.shape[0]):
                ...
                run_error_statistics[init_idx, station_idx, :] = np.array((out[item].item(), cosmo_temperature, target[item].item(), diff[item].item()))
        ...
        da = xr.DataArray(run_error_statistics, dims=('init', 'station', 'data'),
                          coords=[all_inits, stations, ['prediction', 'cosmo', 'target', 'difference']])
